...
const TIME_ZONE = DEFAULT_AGENDA_TIME_ZONE;
...
const statusMeta: Record<AgendaAppointmentStatus, { label: string; tone: 'gold' | 'emerald' | 'danger' | 'neutral' }> = {
...
const formatTime = (timestamp: string) => timeFormatter.format(new Date(timestamp));
...
const Agenda: React.FC = () => {
  const [storedView, setStoredView] = useLocalStorage<AgendaView>('barberzap:agenda-view', 'timeline');
  ...
  const timelineItems = useMemo(
    () => getTimelineItems(visibleAppointments, TIME_ZONE),
    [visibleAppointments]
  );
  const selectedDateLabel = dateFormatter.format(new Date(`${selectedDate}T12:00:00Z`));
  ...
  return (
    <div className="space-y-6 pb-24 text-[#1A1A1F] animate-in slide-in-from-left duration-500">
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <label className="relative flex h-11 min-w-0 items-center gap-2 rounded-xl border border-[#E5E7EB] bg-white px-3 text-sm font-semibold text-[#1A1A1F] focus-within:ring-2 focus-within:ring-[#D4AF37]/50 sm:min-w-[176px]">
              <span className="material-symbols-outlined text-[18px] text-[#9A7417]">calendar_today</span>
            ...
            <div className="flex w-full flex-wrap gap-2 pt-1 sm:w-auto sm:pt-0">
              {[
                { label: 'Ontem', date: addCalendarDays(formatInputDate(new Date()), -1) },
                { label: 'Hoje', date: formatInputDate(new Date()) },
                { label: 'Amanhã', date: addCalendarDays(formatInputDate(new Date()), 1) },
              ].map(({ label, date }) => (
                <button
                  key={label}
                  type="button"
          ...
          <div className="grid grid-cols-3 rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] p-1" role="group" aria-label="Modo de visualização">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <MetricCard icon="calendar_month" label="Agendados" value={loading ? '—' : metrics.total} detail="No dia selecionado" />
        <MetricCard icon="check_circle" label="Confirmados" value={loading ? '—' : metrics.confirmed} detail="Com presença confirmada" accent="emerald" />
      ...
      {loading ? (
        <Panel className="px-5 py-12 text-center" aria-live="polite">
          <span className="material-symbols-outlined mb-3 animate-spin text-3xl text-[#B38D1C]">progress_activity</span>
          <p className="font-semibold text-[#1A1A1F]">Carregando agenda…</p>
          <p className="mt-1 text-sm text-[#6B7280]">Buscando os horários de {selectedDateLabel}.</p>
        </Panel>
      ) : error ? (
        <EmptyPremium
          icon="error"
        ...
        <TimelineView items={timelineItems} onSelect={setSelectedAppointment} />
      ) : view === 'list' ? (
        <ListView appointments={visibleAppointments} onSelect={setSelectedAppointment} />
      ) : (
        <CardsView appointments={visibleAppointments} onSelect={setSelectedAppointment} />
      )}
...
const AppointmentIdentity: React.FC<{ appointment: AgendaAppointment; compact?: boolean }> = ({ appointment, compact = false }) => {
  const clientName = appointment.clientName ?? 'Cliente não informado';
  const avatarUrl = appointment.client?.avatarUrl;
  return (
    <div className="flex min-w-0 items-center gap-3">
      {avatarUrl ? (
        <img src={avatarUrl} alt="" className={`shrink-0 rounded-full border border-[#D4AF37]/35 object-cover ${compact ? 'h-9 w-9' : 'h-11 w-11'}`} />
      ) : (
        <div className={`flex shrink-0 items-center justify-center rounded-full bg-[#F3F4F6] text-[#6B7280] ${compact ? 'h-9 w-9' : 'h-11 w-11'}`}>
          <span className="material-symbols-outlined text-[20px]">person</span>
        </div>
      )}
      <div className="min-w-0 text-left">
        <p className="truncate font-semibold text-[#1A1A1F]">{clientName}</p>
        <p className="truncate text-xs text-[#6B7280]">{appointment.serviceName ?? 'Serviço não informado'}</p>
      </div>
    </div>
  );
};

const AppointmentSummary: React.FC<{ appointment: AgendaAppointment; onSelect: (appointment: AgendaAppointment) => void }> = ({ appointment, onSelect }) => {
    ...
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        <div className="flex min-w-[84px] items-baseline gap-2 border-b border-[#E5E7EB] pb-3 sm:block sm:border-b-0 sm:border-r sm:pb-0 sm:pr-4">
          <p className="text-xl font-bold tabular-nums text-[#1A1A1F]">{formatTime(appointment.startsAt)}</p>
          <p className="text-xs text-[#6B7280]">{durationLabel(appointment.durationMinutes)}</p>
        </div>
        <div className="min-w-0 flex-1"><AppointmentIdentity appointment={appointment} /></div>
        <div className="flex flex-wrap items-center gap-3 sm:justify-end">
          <span className="text-sm font-semibold text-[#1A1A1F]">{formatCurrency(appointment.price)}</span>
          <StatusBadge label={status.label} tone={status.tone} />
          <span className="material-symbols-outlined text-[#9CA3AF]" aria-hidden="true">chevron_right</span>
        </div>
      </div>
    </button>
  );
...
const ListView: React.FC<{ appointments: AgendaAppointment[]; onSelect: (appointment: AgendaAppointment) => void }> = ({ appointments, onSelect }) => (
  <section aria-label="Agenda em lista" className="space-y-3">
    {appointments.map((appointment) => <AppointmentSummary key={appointment.id} appointment={appointment} onSelect={onSelect} />)}
  </section>
);

const CardsView: React.FC<{ appointments: AgendaAppointment[]; onSelect: (appointment: AgendaAppointment) => void }> = ({ appointments, onSelect }) => (
      ...
      return (
        ...
        >
          <div className="mb-4 flex items-start justify-between gap-3">
            <div><p className="text-2xl font-bold tabular-nums text-[#1A1A1F]">{formatTime(appointment.startsAt)}</p><p className="mt-0.5 text-xs text-[#6B7280]">{durationLabel(appointment.durationMinutes)}</p></div>
            <StatusBadge label={status.label} tone={status.tone} />
          </div>
          ...
          <div className="mt-4 flex items-center justify-between gap-3 border-t border-[#E5E7EB] pt-3 text-xs text-[#6B7280]">
            <span className="truncate">{appointment.professional?.name ?? 'Profissional não informado'}</span>
            <span className="shrink-0 font-semibold text-[#1A1A1F]">{formatCurrency(appointment.price)}</span>
          </div>
        </button>
      );
    })}
...
const TimelineView: React.FC<{ items: TimelineItem[]; onSelect: (appointment: AgendaAppointment) => void }> = ({ items, onSelect }) => {
  ...
  return (
    <Panel className="overflow-hidden p-0">
      <div className="border-b border-[#E5E7EB] px-4 py-3 sm:px-5">
        ...
        <p className="mt-0.5 text-xs text-[#6B7280]">Horários que se sobrepõem são exibidos lado a lado.</p>
      </div>
        ...
        <div className="min-w-[620px] p-3 sm:p-4">
          <div className="grid grid-cols-[52px_minmax(0,1fr)]">
            <div className="relative" style={{ height: timelineHeight }}>
              {hours.map((hour) => (
                <span key={hour} className="absolute right-3 -translate-y-1/2 text-xs font-medium tabular-nums text-[#6B7280]" style={{ top: (hour - startHour) * 60 * pixelsPerMinute }}>
                  {String(hour).padStart(2, '0')}:00
                </span>
              ))}
            </div>
            <div className="relative border-l border-[#E5E7EB] bg-[linear-gradient(to_bottom,rgba(229,231,235,0.8)_1px,transparent_1px)]" style={{ height: timelineHeight, backgroundSize: `100% ${60 * pixelsPerMinute}px` }}>
              {items.map((item) => {
                const top = (item.startMinutes - startHour * 60) * pixelsPerMinute;
                const height = Math.max((item.endMinutes - item.startMinutes) * pixelsPerMinute - 5, 48);
                return (
                  <button
                    key={item.appointment.id}
                    type="button"
                    onClick={() => onSelect(item.appointment)}
                    aria-label={`${formatTime(item.appointment.startsAt)}, ${item.appointment.clientName ?? 'cliente não informado'}, ${item.appointment.serviceName ?? 'serviço não informado'}`}
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <span className="truncate text-sm font-bold text-[#1A1A1F]">{item.appointment.clientName ?? 'Cliente não informado'}</span>
                      <span className="shrink-0 text-[11px] font-semibold tabular-nums text-[#7A5E12]">{formatTime(item.appointment.startsAt)}</span>
                    </div>
                    <p className="truncate text-xs text-[#6B7280]">{item.appointment.serviceName ?? 'Serviço não informado'}</p>
                    <p className="mt-1 truncate text-[11px] text-[#7A5E12]">{durationLabel(item.appointment.durationMinutes)}</p>
                  </button>
                );
              })}
            </div>
...
const Detail: React.FC<{ icon: string; label: string; children: React.ReactNode }> = ({ icon, label, children }) => (
  <div className="rounded-2xl border border-[#E5E7EB] bg-[#FCFCFD] p-4">
    <p className="mb-1.5 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6B7280]"><span className="material-symbols-outlined text-[16px] text-[#9A7417]">{icon}</span>{label}</p>
    <div className="text-sm leading-6 text-[#1A1A1F]">{children}</div>
  </div>
);

const AppointmentDetailsModal: React.FC<{ appointment: AgendaAppointment | null; onClose: () => void }> = ({ appointment, onClose }) => {
  if (!appointment) return null;
  const status = statusMeta[appointment.status];
  const contact = [appointment.client?.phone, appointment.client?.email].filter(Boolean).join(' · ');
  return (
    <Modal isOpen onClose={onClose} title="Detalhes do agendamento" eyebrow="Agenda diária" size="lg">
