...
const MobileMoreSheet: React.FC<MobileMoreSheetProps> = ({
  ...
  useEffect(() => {
    ...
    if (routeChanged && isOpen) onClose();
  ...
  return (
    <>
      ...
      <div
        ...
        tabIndex={-1}
        className="bz-mobile-sheet fixed inset-x-0 bottom-0 z-50 max-h-[calc(100dvh-1rem)] overscroll-contain overflow-y-auto rounded-t-3xl border-t border-[#E5E7EB] bg-white px-5 pt-3 shadow-[0_-24px_60px_rgba(15,23,42,0.18)] outline-none"
      >
        ...
        <div className="mb-5 flex items-center justify-between gap-4">
          <div>
            ...
            <h2 id="mobile-more-title" className="bz-title-serif text-lg text-[#1A1A1F]">
              Mais opções
            </h2>
          </div>
          <button
        ...
        <div className="space-y-6">
          {groups.map((group) => (
            <section key={group.id} aria-labelledby={`mobile-more-group-${group.id}`}>
              <p
                id={`mobile-more-group-${group.id}`}
                className="mb-2 px-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[#9CA3AF]"
              >
                {group.label}
              </p>
