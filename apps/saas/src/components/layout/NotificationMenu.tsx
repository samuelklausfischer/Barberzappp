...
import { formatNotificationDateTime, getUnreadNotificationLabel } from '@/features/notifications/notificationUtils';
    ...
    <div ref={menuRef} className="relative">
      ...
      {isOpen && (
        <section
          ...
          className="absolute right-0 top-[calc(100%+0.6rem)] z-50 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white shadow-[0_18px_48px_rgba(15,23,42,0.16)]"
        >
          <div className="flex items-center justify-between gap-3 border-b border-[#E5E7EB] px-4 py-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-[#6B7280]">{getUnreadNotificationLabel(unreadCount)}</p>
            </div>
            {unreadCount > 0 && (
              <button
                type="button"
          ...
          <div className="max-h-[min(28rem,calc(100vh-9rem))] overflow-y-auto">
            {loading && notifications.length === 0 && <p className="px-4 py-8 text-center text-sm text-[#6B7280]">Carregando notificações…</p>}
            {!loading && error && notifications.length === 0 && (
              <div className="px-4 py-7 text-center">
                <p className="text-sm text-[#B42318]">{error}</p>
                <button type="button" onClick={() => void refresh()} className="mt-3 min-h-10 rounded-lg px-3 text-sm font-semibold text-[#8B6B12] hover:bg-[#D4AF37]/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#D4AF37]">
                  Tentar novamente
                </button>
              </div>
            )}
            {!loading && error && notifications.length > 0 && (
              <p className="border-b border-[#FEE4E2] bg-[#FFFAEB] px-4 py-2 text-center text-xs text-[#B42318]">{error}</p>
            )}
            {!loading && !error && notifications.length === 0 && (
              <div className="px-4 py-10 text-center">
                <span className="material-symbols-outlined text-[30px] text-[#A0A7B3]" aria-hidden="true">notifications_none</span>
                <p className="mt-2 text-sm font-semibold text-[#1A1A1F]">Tudo em dia</p>
                <p className="mt-1 text-xs text-[#6B7280]">Os próximos agendamentos aparecerão aqui.</p>
              </div>
            )}
            {!loading && !error && notifications.map((notification) => (
              ...
              >
                <span className={`mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full ${notification.readAt ? 'bg-[#D8DDE5]' : 'bg-[#B38D1C]'}`} aria-hidden="true" />
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-0.5 block truncate text-xs text-[#5E6673]">
                    {notification.serviceName || 'Serviço a confirmar'} · {formatNotificationDateTime(notification.scheduledAt, timeZone)}
                  </span>
                  {!notification.readAt && <span className="mt-1 block text-[11px] font-semibold text-[#8B6B12]">Não lida</span>}
                </span>
              </button>
            ))}
