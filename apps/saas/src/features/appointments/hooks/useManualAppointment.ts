...
import {
  ...
  type ManualAppointmentService,
  type WorkingHoursInterval,
} from '../utils/manualAppointment';

type ClientOption = { id: string; name: string; phone: string | null };
export type BarberOption = { id: string; name: string; workingHours: WorkingHoursInterval[] };
...
const mapWorkingHours = (rows: WorkingHoursRow[] | null) =>
  (rows ?? [])
    .filter(
      (
        row
      ): row is WorkingHoursRow & { day_of_week: number; start_time: string; end_time: string } =>
    ...
    .map((row): WorkingHoursInterval | null => {
      const startTime = normalizeWorkingHoursTime(row.start_time);
      const endTime = normalizeWorkingHoursTime(row.end_time);
      return startTime && endTime
        ? {
            dayOfWeek: row.day_of_week,
            startTime,
            endTime,
            isActive: isWorkingHoursRowActive(row.is_active),
            timezone: row.timezone,
          }
        : null;
    })
    .filter((interval): interval is WorkingHoursInterval => interval !== null)
    .filter(isValidWorkingHoursInterval)
    .sort(
      (left, right) =>
...
export const mapManualAppointmentRpcError = (cause: unknown) => {
  ...
  if (message.includes('PAST_APPOINTMENT'))
  ...
  if (message.includes('CLIENT_NOT_FOUND'))
    return 'O cliente selecionado n\u00e3o est\u00e1 dispon\u00edvel.';
  if (message.includes('BARBER_NOT_FOUND'))
    return 'O barbeiro selecionado n\u00e3o est\u00e1 dispon\u00edvel.';
  if (message.includes('BARBER_OUTSIDE_WORKING_HOURS'))
    return 'O profissional n\u00e3o atende neste hor\u00e1rio.';
  if (message.includes('SERVICE'))
    return 'Um ou mais servi\u00e7os selecionados n\u00e3o est\u00e3o dispon\u00edveis.';
  return 'N\u00e3o foi poss\u00edvel criar o agendamento. Tente novamente.';
};

export const useManualAppointmentOptions = () => {
  const { tenantId } = useTenant();
  const [options, setOptions] = useState<ManualAppointmentOptions>({
    clients: [],
    services: [],
    barbers: [],
  });
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [clientsResult, servicesResult, barbersResult] = await Promise.all([
        supabase
          .from('clients')
          .select('id, name, phone, deleted_at, status')
          .eq('tenant_id', tenantId)
          .is('deleted_at', null)
          .order('name'),
        supabase
          .from('services')
          .select('id, name, price, duration, duration_minutes, active, status, barber_id')
          .eq('tenant_id', tenantId)
        ...
        supabase
          .from('barbers')
          .select(
            'id, name, active, status, working_hours!working_hours_barber_id_fkey(day_of_week, start_time, end_time, is_active, timezone)'
          )
          .eq('tenant_id', tenantId)
          .or('active.is.null,active.eq.true')
          .order('name'),
      ...
      setOptions({
        ...
        services: ((servicesResult.data ?? []) as ServiceRow[])
          .map((row) => ({
            row,
            durationMinutes: getServiceDurationMinutes(row.duration_minutes, row.duration),
          }))
          .filter(
            ({ row, durationMinutes }) =>
        ...
        barbers: ((barbersResult.data ?? []) as BarberRow[])
          ...
          .map((row) => ({
            id: String(row.id),
            name: row.name ?? 'Barbeiro sem nome',
            workingHours: mapWorkingHours(row.working_hours),
          }))
          .filter((barber) => barber.workingHours.length > 0),
      });
    } catch {
      if (request !== requestRef.current) return;
      setOptions({ clients: [], services: [], barbers: [] });
  ...
  const create = useCallback(
    async ({
      ...
      try {
        ...
        if (!result?.success)
          throw new Error(result?.code ?? result?.message ?? 'VALIDATION_ERROR');
        return true;
      } catch (cause) {
